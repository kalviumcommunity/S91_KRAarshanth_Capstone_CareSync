...
export const makeCrud=(Model,opts={})=>({
 list:asyncHandler(async(req,res)=>{const {page=1,limit=50,search}=req.query;const filter=search&&opts.searchFields?.length?{$or:opts.searchFields.map(f=>({[f]:{$regex:search,$options:'i'}}))}:{};let query=Model.find(filter).sort({createdAt:-1}).limit(Math.min(Number(limit),100)).skip((Number(page)-1)*Number(limit));if(opts.populate)query=query.populate(opts.populate);const data=await query;const total=await Model.countDocuments(filter);res.json({success:true,data,pagination:{page:Number(page),limit:Number(limit),total,totalPages:Math.ceil(total/Number(limit))}})}),
 get:asyncHandler(async(req,res)=>{let query=Model.findById(req.params.id);if(opts.populate)query=query.populate(opts.populate);const item=await query;if(!item)throw new ApiError(404,`${Model.modelName} not found`);res.json({success:true,data:item})}),
